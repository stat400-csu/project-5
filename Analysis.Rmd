...
```
Leauge Level Averages



## Replicating the MVP Ranking with Linear Regression
Rank 1 → 10 points
...
mvp_model <- lm(points ~ pts_per_g + trb_per_g + ast_per_g, data = df)
...
df <- df %>%
  mutate(predicted_points = predict(mvp_model, newdata = df)) %>%
  arrange(desc(predicted_points))

df %>% select(namePlayer, rank, points, predicted_points)


ggplot(df, aes(x = points, y = predicted_points, label = namePlayer)) + geom_point(color = "forestgreen", size = 3) + 
  geom_text(nudge_y = 0.5) +
  labs(title = "Predicted vs Proxy MVP Points (Ranked by PTS per Game)",
...
ggplot(pred_summary, aes(x = actual_points, y = avg_pred, label = player)) +
