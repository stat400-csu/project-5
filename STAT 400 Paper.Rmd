...
To construct the confusion matrix, to be able to more effectively fit the matrix into the confines of what was covered in this class, instead of using machine learning models, Monte Carlo simulation and bootstrapping were employed.
...
Finally, the matrix that uses bootstrapped sampling with Monte Carlo averages is considered.

```{r}

```

This matrix, as can be expected, is found to be quite similar to the regular bootstrapped matrix, with its differences being within the hundredths of correlation values.
Using Monte Carlo methods, although greatly effective in many applications, have little effect on correlations when specifically finding means on our bootstrapped data.
